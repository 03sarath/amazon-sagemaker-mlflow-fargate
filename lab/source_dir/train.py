...
if __name__ =='__main__':
    ...
    with mlflow.start_run():
        ...
        # Original ABS ERROR metrics
        abs_err = np.abs(y_pred - y_test)
        for q in [10, 50, 90]:
            percentile_value = np.percentile(a=abs_err, q=q)
            logging.info(f'AE-at-{q}th-percentile: {percentile_value}')
            mlflow.log_metric(f'AE-at-{str(q)}th-percentile', percentile_value)
        
        # Optional: Log training metrics as well
